...
while true; do
  ...
  if [ -n "$IP" ]; then
    TOKEN=$(kubectl exec -n ${var.namespace} github-action-master-${var.namespace} -- cat /var/lib/rancher/k3s/server/node-token 2>/dev/null)
    if [ -n "$TOKEN" ]; then
...
output "k3s_master_token" {
  value = data.external.k3s_master_info.result["token"]
}
